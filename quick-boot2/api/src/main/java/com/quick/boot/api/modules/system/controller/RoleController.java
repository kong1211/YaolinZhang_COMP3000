...
public class RoleController {
    ...
    public Restful<Page<Role>> roleList(PageQuery query) {
        logService.addLog((String) SecurityUtils.getSubject().getPrincipal(), "Viewrole列表");
    ...
    public Restful<Void> saveRole(@RequestBody Role role) {
        ...
        logService.addLog((String) SecurityUtils.getSubject().getPrincipal(), "保存role信息 ID=>" + role.getId());
    ...
    public Restful<Void> deleteRole(@PathVariable Integer id) {
        ...
        logService.addLog((String) SecurityUtils.getSubject().getPrincipal(), "deleterole信息 ID=>" + id);
        return Restful.ok(null);
    }
}
